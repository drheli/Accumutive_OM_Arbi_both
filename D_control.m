...
function [diag,count,k,u_r,n_r,d_plus_in, d_minus_in,d_o_plus,d_o_minus] = D_control(p_plus,p_minus)
unrolling = 8;
delta = 3;
persistent i; 
persistent j;
...
if diag_c - ite_k == 1
    ...
    i = i + 1;   
    u_r=i;
        d_plus_in = d_plus_w(pairing(0,ite_k),i);
        d_minus_in = d_minus_w(pairing(0,ite_k),i);
    if i == delta + 1
        count_c = count_c + 1;
        i = 0;
    end
    ...
    j = j+1;
        ...
        %u_r = mod (w,unrolling);
        %d_minus_in = d_minus(ite_k,(delta+1)+(diag_c-ite_k)*delta + j);
        d_plus_in = d_plus_w(pairing(n_r,ite_k),u_r);
        d_minus_in = d_minus_w(pairing(n_r,ite_k),u_r);
    %end
    if j == delta
        count_c = count_c + 1;
        j = 0;
